...
translations
  "_unrest x p" == "CONST unrest x U(p)"                                           
...
lemma unrest_uop [unrest]:
  "\<lbrakk> x \<sharp> @e \<rbrakk> \<Longrightarrow> x \<sharp> f (@e)"
  by (auto simp add: expr_defs)

lemma unrest_bop [unrest]:
  "\<lbrakk> x \<sharp> @e\<^sub>1; x \<sharp> @e\<^sub>2 \<rbrakk> \<Longrightarrow> x \<sharp> f (@e\<^sub>1) (@e\<^sub>2)"
  by (auto simp add: expr_defs)

lemma unrest_trop [unrest]:
  "\<lbrakk> x \<sharp> @e\<^sub>1; x \<sharp> @e\<^sub>2; x \<sharp> @e\<^sub>3 \<rbrakk> \<Longrightarrow> x \<sharp> f (@e\<^sub>1) (@e\<^sub>2) (@e\<^sub>3)"
  by (auto simp add: expr_defs)
