...
definition subst_lookup :: "('s\<^sub>1,'s\<^sub>2) psubst \<Rightarrow> ('a \<Longrightarrow> 's\<^sub>2) \<Rightarrow> ('a, 's\<^sub>1) expr" ("\<langle>_\<rangle>\<^sub>s")
  where [expr_defs]: "\<langle>\<sigma>\<rangle>\<^sub>s = (\<lambda> x b. get\<^bsub>x\<^esub> (\<sigma> b))"
...
(*  "_SubstUpd m (_smaplet x y)"        <= "CONST subst_upd m x y" *)
  ...
  "_psubst m x v"  => "CONST subst_upd m x U(v)"
  "_subst P v x" <= "CONST subst_app (CONST subst_upd id\<^sub>s x U(v)) P"
(*
...
lemma subst_id_var: "id\<^sub>s = U(&\<^bold>v)"
  by (simp add: expr_defs lens_defs)

lemma subst_upd_id_lam [usubst]: "subst_upd U(&\<^bold>v) x v = subst_upd id\<^sub>s x v"
...
setup \<open> nolift_const @{const_name subst_app} \<close>

translations 
  "CONST subst_app \<sigma> e" <= "CONST subst_app \<sigma> e _sexp_state"

term "U(f (\<sigma> \<dagger> @e))"

term "U(\<forall> x. x + &y > &z)"

term "U(\<forall> k. P\<lbrakk>k/x\<rbrakk>)"

lemma subst_var [usubst]: "\<sigma> \<dagger> U(&x) = \<langle>\<sigma>\<rangle>\<^sub>s x"
  by (simp add: expr_defs)
...
lemma subst_uop: "\<sigma> \<dagger> U(f (@e)) = U(f (\<sigma> \<dagger> U(@e)))"
  by (simp add: expr_defs)

lemma subst_bop: "\<sigma> \<dagger> U(f (@e\<^sub>1) (@e\<^sub>2)) = U(f (\<sigma> \<dagger> U(@e\<^sub>1)) (\<sigma> \<dagger> U(@e\<^sub>2)))"
  by (simp add: expr_defs)
...
lemma subst_lookup_upd [usubst]:
  assumes "weak_lens x"
  shows "\<langle>\<sigma>(x \<mapsto>\<^sub>s v)\<rangle>\<^sub>s x = U(v)"
